...
class ComprehensiveAnalyzer(ast.NodeVisitor):
    ...
    def visit_Module(self, node):
        ...
        self.file_info.total_lines = max(
            [n.lineno for n in ast.walk(node) if hasattr(n, "lineno")] + [0]
        )
    ...
    def visit_Call(self, node):
        ...
        if func_name:
            ...
            current_scope_key = (
                ".".join(self.current_scope) if self.current_scope else "__module__"
            )
    ...
    def visit_ClassDef(self, node):
        ...
        is_abstract = any(
            "abc" in str(base) or "ABC" in str(base) for base in node.bases
        )
    ...
    def _analyze_function(self, node, is_method=False, is_async=False) -> FunctionInfo:
        ...
        scope_key = (
            f"{'.'.join(self.current_scope)}.{node.name}"
            if self.current_scope
            else node.name
        )
        self.current_scope.append(node.name)
...
def build_project_tree(data: Dict[str, FileInfo]) -> Dict[str, Any]:
    ...
    for file_path, info in data.items():
        ...
        node[filename] = {
            "_metadata": {
                "classes": [
                    (c.name, c.line_number, len(c.methods), c.is_abstract)
                    for c in info.classes
                ],
                "functions": [
                    (f.name, f.line_number, f.complexity_score, f.is_async)
    ...
    for idx, key in enumerate(keys):
        ...
        if metadata:  # It's a file
            ...
            complexity_indicator = (
                "🔴" if complexity > 20 else "🟡" if complexity > 10 else "🟢"
            )
            size_indicator = (
                "📚" if lines_count > 500 else "📄" if lines_count > 100 else "📃"
            )

            lines.append(
                f"{prefix}{connector}{size_indicator} {key} {complexity_indicator} "
                f"({lines_count} lines, complexity: {complexity})"
            )

            # Show classes with method counts
            for cls_name, line_num, method_count, is_abstract in metadata.get(
                "classes", []
            ):
                abstract_indicator = "🔶" if is_abstract else "🏛️"
                lines.append(
                    f"{prefix}{'    ' if is_last else '│   '}├── {abstract_indicator} {cls_name} "
                    f"(line {line_num}, {method_count} methods)"
                )

            # Show functions with complexity
            for func_name, line_num, func_complexity, is_async in metadata.get(
                "functions", []
            ):
                async_indicator = "⚡" if is_async else "⚙️"
                func_complexity_indicator = (
                    "🔴"
                    if func_complexity > 10
                    else "🟡" if func_complexity > 5 else "🟢"
                )
                lines.append(
                    f"{prefix}{'    ' if is_last else '│   '}└── {async_indicator} {func_name} "
        ...
        if any(k != "_metadata" for k in child.keys()):
            subtree_lines = generate_tree_visualization(
...
def generate_comprehensive_markdown(
    tree_lines: List[str], data: Dict[str, FileInfo]
) -> str:
    ...
    total_methods = sum(
        len(cls.methods) for info in data.values() for cls in info.classes
    )
    total_lines = sum(info.total_lines for info in data.values())
    avg_complexity = (
        sum(info.complexity_score for info in data.values()) / len(data) if data else 0
    )
    ...
    md.append(
        f"- **Most Complex Files:** {', '.join([f for f, info in sorted(data.items(), key=lambda x: x[1].complexity_score, reverse=True)[:3]])}"
    )
    md.append("")
    ...
    for file_path, info in sorted(data.items()):
        ...
        complexity_indicator = (
            ...
            else "🟡" if info.complexity_score > 10 else "🟢"
        )
        md.append(
            f"**Overview:** {info.total_lines} lines, complexity: {info.complexity_score} {complexity_indicator}\n"
        )
        ...
        if info.global_variables or info.constants:
            ...
            if info.constants:
                ...
                for const in info.constants:
                    md.append(
                        f"- `{const.name}` = {const.assigned_values[0] if const.assigned_values else 'undefined'} (line {const.line_number})"
                    )
                md.append("")
            ...
            if info.global_variables:
                ...
                for var in info.global_variables:
                    md.append(
                        f"- `{var.name}` = {var.assigned_values[0] if var.assigned_values else 'undefined'} (line {var.line_number})"
                    )
                md.append("")
        ...
        if info.classes:
            ...
            for cls in info.classes:
                ...
                decorators_info = (
                    f" @{', @'.join(cls.decorators)}" if cls.decorators else ""
                )

                md.append(
                    f"#### `{cls.name}`{inheritance_info}{abstract_info} - lines {cls.line_number}-{cls.end_line_number}{decorators_info}"
                )

                if cls.doc:
                    md.append(
                        f"**Purpose:** {cls.doc[:150]}{'...' if len(cls.doc) > 150 else ''}\n"
                    )
                ...
                if cls.class_variables:
                    ...
                    for var in cls.class_variables:
                        md.append(
                            f"- `{var.name}` = {var.assigned_values[0] if var.assigned_values else 'undefined'}"
                        )
                    md.append("")
                ...
                if cls.methods:
                    ...
                    for method in cls.methods:
                        ...
                        complexity_ind = (
                            ...
                            else "🟡" if method.complexity_score > 5 else "🟢"
                        )
                        decorators_str = (
                            f" @{', @'.join(method.decorators)}"
                            if method.decorators
                            else ""
                        )

                        md.append(
                            f"- `{async_str}{method.name}{params_str}{return_str}` {complexity_ind} "
                            f"(lines {method.line_number}-{method.end_line_number}, complexity: {method.complexity_score}){yield_str}{decorators_str}"
                        )

                        if method.doc:
                            md.append(
                                f"  - 📝 {method.doc[:100]}{'...' if len(method.doc) > 100 else ''}"
                            )

                        if method.calls:
                            calls_list = sorted(method.calls)[:5]
                            more_calls = (
                                f" (+{len(method.calls)-5} more)"
                                if len(method.calls) > 5
                                else ""
                            )
                            md.append(
                                f"  - 🔗 Calls: `{', '.join(calls_list)}`{more_calls}"
                            )

                        if method.subfunctions:
                            md.append(
                                f"  - 🔧 Nested functions: `{', '.join(method.subfunctions)}`"
                            )
                    md.append("")
        ...
        if info.functions:
            ...
            for func in info.functions:
                ...
                complexity_ind = (
                    ...
                    else "🟡" if func.complexity_score > 5 else "🟢"
                )
                decorators_str = (
                ...
                md.append(
                    f"#### `{async_str}{func.name}{params_str}{return_str}` {complexity_ind} "
                    f"(lines {func.line_number}-{func.end_line_number}, complexity: {func.complexity_score}){yield_str}{decorators_str}"
                )

                if func.doc:
                    md.append(
                        f"**Purpose:** {func.doc[:150]}{'...' if len(func.doc) > 150 else ''}"
                    )

                if func.calls:
                    calls_list = sorted(func.calls)[:8]
                    more_calls = (
                        f" (+{len(func.calls)-8} more)" if len(func.calls) > 8 else ""
                    )
                    md.append(
                        f"- 🔗 Function calls: `{', '.join(calls_list)}`{more_calls}"
                    )

                if func.subfunctions:
                    md.append(
                        f"- 🔧 Nested functions: `{', '.join(func.subfunctions)}`"
                    )

                if func.variables:
                    var_names = [v.name for v in func.variables[:5]]
                    more_vars = (
                        f" (+{len(func.variables)-5} more)"
                        if len(func.variables) > 5
                        else ""
                    )
                    md.append(
                        f"- 📊 Local variables: `{', '.join(var_names)}`{more_vars}"
                    )
        ...
        if info.external_references:
            ...
            for module, refs in sorted(external_by_module.items()):
                if len(refs) <= 10:
                    ...
                    md.append(
                        f"- **{module}**: `{', '.join(sorted(refs)[:10])}` (+{len(refs)-10} more)"
                    )
            md.append("")
        ...
        if high_complexity_funcs:
            ...
            modification_hints.append(
                f"**Refactoring candidates:** Functions `{', '.join(func_names)}` have high complexity and could benefit from decomposition"
            )
        ...
        if undocumented_funcs:
            ...
            more_str = (
                f" (+{len(undocumented_funcs)-5} more)"
                if len(undocumented_funcs) > 5
                else ""
            )
            modification_hints.append(
                f"**Documentation needed:** Functions `{', '.join(func_names)}`{more_str} lack docstrings"
            )
        ...
        if undocumented_classes:
            ...
            modification_hints.append(
                f"**Class documentation:** Classes `{', '.join(class_names)}` need docstrings"
            )

        # Large files
        if info.total_lines > 500:
            modification_hints.append(
                f"**File size:** This file has {info.total_lines} lines, consider splitting into smaller modules"
            )
        ...
        if complex_signatures:
            ...
            modification_hints.append(
                f"**Parameter complexity:** Functions `{', '.join(func_names)}` have many parameters, consider using dataclasses or configuration objects"
            )
...
def generate_cross_reference_analysis(data: Dict[str, FileInfo]) -> str:
    ...
    if external_calls:
        ...
        for module, calls in sorted(by_module.items()):
            if len(calls) <= 5:
                md.append(
                    f"- **{module}**: {len(calls)} calls - `{', '.join(sorted(calls))}`"
                )
            else:
                md.append(
                    f"- **{module}**: {len(calls)} calls - `{', '.join(sorted(calls)[:5])}` (+{len(calls)-5} more)"
                )
        md.append("")
...
Examples:
  ...
  python ai_code_mapper.py --root . --verbose
    ...
    markdown_content = generate_comprehensive_markdown(
        tree_visualization, analysis_data
    )
...
"""
    ...
    try:
        ...
        logger.info(
            f"📊 Analysis summary: {len(analysis_data)} files, "
        ...
        high_complexity_files = [
            f
            for f, info in analysis_data.items()
            if info.complexity_score > args.max_complexity
        ]
        if high_complexity_files:
            logger.warning(
                f"⚠️ High complexity files detected: {', '.join(high_complexity_files)}"
            )
